...
MODEL_SAVE_PATH_ALL_number = os.getcwd() + "/model/number/"
MODEL_NAME21 = "modelckpt"
data_dir = './final/%s/'
test_dir = './test/%s'
RANGE_DIR = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "0", "P", "R", "S", "T", "U", "V", "X",
             "Y","Z"]
class_num = len(RANGE_DIR)
...
def train():
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(inception_v3_arg_scope()):
            y, _ = inception_v3(x, class_num)
            # y_test, _ = inception_v3(x, class_num, False)
        ...
        with tf.Session() as sess:
            ...
            for i in range(max_step):
                image, label = readdata.batch(32)
                train_dict = {x: image, y_: label}
                if i%10  ==0:
                    loss_,accuracy_ = sess.run([loss,accuracy],feed_dict=train_dict)
                    print("after %d train step, loss on trainable  batch is  %g,accuracy is %g" % (i, loss_,accuracy_))
                sess.run(train_step,feed_dict=train_dict)
                if i%100 ==0:
                    image_,label_ = readdata.test(32)
                    test_dict = {x:image_,y_:label_}
                    loss_,accuracy_ = sess.run([loss,accuracy],feed_dict=test_dict)
                    print("after %d train step, loss on test  is  %g,accuracy is %g" % (i, loss_,accuracy_))
